    ...
    Datepicker.prototype = {
        ...
        init: function init() {
            ...
            if (this.inline) {
                ...
                var top = this.ele.offsetHeight + 5,
                    left = 0;
        ...
        initEvent: function initEvent() {
            var that = this,
                wrap = !this.inline ? this.ele : this.wrap;
            wrap.addEventListener('click', function (e) {
                var target = e.target,
                    eventType = target.getAttribute('dp-e');
        ...
        slideHandler: function slideHandler(step) {
        ...
        titleHandler: function titleHandler() {
            this.switchShowPanel(1);
        },
        switchShowPanel: function switchShowPanel(dir) {
            var panelName = this.showPanel || 'day',
                panelMap = dir ? { day: 'month', month: 'year' } : { year: 'month', month: 'day' },
                hidePanel = null,
                showPanel = null;
        ...
        tdHandler: function tdHandler(target) {
            var type = this.showPanel || 'day',
        ...
        getMonthPanelTpl: function getMonthPanelTpl(date) {
            var curDate = date || this.focusDate,
                year = curDate.getFullYear(),
                month = curDate.getMonth(),
                locale = Datepicker.locale,
                dateArr = locale.MONTH,
                tbody = '',
            ...
            for (var i = 0, len = dateArr.length; i < len; i++) {
                cls = '';
                if (new Date(year, i + 1, 0).getTime() < minTimestamp || new Date(year, i, 1).getTime() > maxTimestamp) {
                    cls += ' disabled';
                }
        ...
        getDayPanelTpl: function getDayPanelTpl(date) {
            var curDate = date || this.focusDate,
                tpl = '',
                ...
                statusArr = [!!(startTs - daySeconds < minTimestamp)],
                cls = '';
            ...
            for (var i = 0; i <= 42; i++) {
                ...
                if (this.showRange === 'start' && startTs >= curTimestamp && startTs <= maxTimestamp || this.showRange === 'end' && startTs <= curTimestamp && startTs >= minTimestamp) cls += ' range';
        ...
        getPanelHeadTpl: function getPanelHeadTpl(type, statusArr, title) {
            var locale = Datepicker.locale,
                colspan = type === 'day' ? 7 : 4,
                thead = '',
                tfoot = '',
        ...
        setFocusDate: function setFocusDate(year, month, day) {
            var oYear = this.focusDate.getFullYear(),
                oMonth = this.focusDate.getMonth(),
                date = parseDate(new Date(0));
            if (!year) {
                year = oYear;
        ...
        setDate: function setDate(date, changeFocus) {
            this.date = date;
            if (changeFocus) {
        ...
        checkDateIsValid: function checkDateIsValid() {
    ...
    RangeDatepicker.prototype = {
        ...
        initEvent: function initEvent() {
            ...
            this.ele.addEventListener('click', function (e) {
                var target = e.target,
                    etype = target.getAttribute('dp-e'),
                    fn;
                if (etype) {
            ...
            this.startDateInput.addEventListener('blur', function () {
                ...
                if (!date || date.toString() === 'Invalid Date' || date.getTime() > maxTimestamp || date.getTime() < minTimestamp) {
        ...
        applyHandle: function applyHandle() {
            var inputStart = this.startDateInput.value,
                inputEnd = this.endDateInput.value;
        ...
        mouseoverEvent: function mouseoverEvent(e) {
            var target = e.target,
                type = target._type,
                item;
        ...
        mouseoutEvent: function mouseoutEvent(e) {
        ...
        setStartValue: function setStartValue(date) {
            date = dateFormat(date, this.startPicker.format);
            this.startDateInput.value = date;
        },
        setEndValue: function setEndValue(date) {
            date = dateFormat(date, this.endPicker.format);
            this.endDateInput.value = date;
        },
        checkShotCut: function checkShotCut() {
            var startDate = this.startPicker.date.getTime(),
                endDate = this.endPicker.date.getTime();
            var shortcut = this._shortCutDate,
                flag = true;
        ...
        changeShortCutStatus: function changeShortCutStatus(i) {
        ...
        showCustomePanel: function showCustomePanel() {
            this.wrap.className += ' daterange-show';
        },
        hideCustomePanel: function hideCustomePanel() {
